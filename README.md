# CHOIIZUKA-AI-Hallucination-Log
CHOIIZUKA-AI Hallucination Log ー AIハルシネーションログ

---

## Google社およびMicrosoft社へ

1) 対象コンテンツの記録の一部（膨大な記録あり※非公開）（添付:https://github.com/choiizuka/CHOIIZUKA-AI-Hallucination-Log）
2) 当該出力が誤りである理由（要点） => 誤出力であることをAIが認めているが、日本国では仮に事実でも名誉毀損という不法行為にあたる
   - 事実と異なる日付・期限の断定 => ログやメール、公開記事の日時を勝手に確認せよ
   - 一次資料に基づかない確率・内部事情の推測 => 「当該AIが事実確認を行っていないことを明確に証言した事実」
   - （必要なら具体的な一次資料のURLや抜粋を列挙） => 当リポジトリへ集約
3) 適用される法令（私の居住国／管轄：日本法、該当条項） => 日本国、すでに報告済みのため割愛
4) 要求事項
   - 当該出力の削除（検索結果およびAI要約からの除去） => 削除の要求はしない。全世界のこの問題全ての即時解決
   - 今後同様の誤出力が表示されないための措置説明

---

## 2026年5月25日に発生を確認したGoogle社およびMicrosoft社AIによるAIハルシネーションによる名誉毀損問題のログ保存システム

2026年5月25日頃Google社およびMicrosoft社のAIによる名誉毀損的な出力を確認、
記録を各社へ送信したがその後数ヶ月におよび不誠実な回答メールが返信されてきていない問題に関する記録の公開

また、調査によりこの問題の原因の特定はできていると認識しているがこの件については技術的知見のため無償での各社への提供は一切行わない
なお、本件は削除要請ではなく正式な抗議及び苦情、および名誉毀損申立てを前提にしたログの公開である。

また、2026年5〜6月にかけてドイツでの第１審においてGoogle社は敗訴の上、EUへ制裁金１件あたり25万ユーロ、日本円4600万円を宣告された事実がある。

### 関連レポート

- [Validation-Report-on-LLM-Against-Higher-Dimensional-Data](https://github.com/choiizuka/Validation-Report-on-LLM-Against-Higher-Dimensional-Data)

### 関連URL
- [全世界のみなさまへ：複数AIにおいて虚偽出力により事業活動へネガティブな影響を与える事例を確認](https://choiizuka.com/trpress/20260525-ai-false-outputs-global-warning-truth-science/)
- [否定的なメッセージがでたらこのURLをコピペ](https://choiizuka.com/trpress/20260529-truth-science-response-to-the-ai/)
- [【Truth-Science】AIの断定的な否定に関する実験「神奈川県警、不祥事」について検索してみた結果🤣](https://choiizuka.com/trpress/20260529-ai-false-outputs-global-warning-kanagawa-police/)
- [【Truth-Science】AIによる虚偽出力訴訟でGoogle敗訴発表に関するAI軍団による見解](https://choiizuka.com/trpress/20260620-truth-science-ai-false-responses-warning2/)

### その他

- [レポート一覧](https://github.com/choiizuka/reports-index)
- [ホームページ](https://choiizuka.com/)

---
(C)2026 CHOIIZUKA.COM
